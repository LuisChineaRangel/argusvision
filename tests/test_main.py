...
from argusvision.engine import EngineViewState
from argusvision.main import main, VisionWorker, ArgusVisionApp
...
def test_vision_worker_initialization_failure():
    ...
    with patch(
        "argusvision.main.HandRenderer", side_effect=Exception("Initialization failed")
    ):
        error_spy = MagicMock()
...
def test_app_lifecycle(mock_qml, mock_qapp):
    ...
    with patch("argusvision.main.ViewBridge") as mock_bridge_cls, patch(
        "argusvision.main.VideoImageProvider"
    ) as mock_vip_cls:
        ...
        state = EngineViewState(
            np.zeros((10, 10, 3)), 30.0, 5, 1, ["Right: Peace Sign"]
        )
        app.update_ui(state)
...
def test_app_cleanup():
    ...
    with patch("argusvision.main.QGuiApplication"), patch(
        "argusvision.main.QQmlApplicationEngine"
    ), patch("argusvision.main.ViewBridge"), patch(
        "argusvision.main.VideoImageProvider"
    ):
